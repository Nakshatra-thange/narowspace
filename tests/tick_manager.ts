...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function getTickData(account: any, absoluteTick: number, arrayStart: number) {
...
describe("tick_manager", () => {
  ...
  describe("update_tick", () => {
    ...
    it("lower tick gets positive liquidity_net", async () => {
      ...
      const tick = getTickData(ta, 0, tickToArrayStartTick(0));
    ...
    it("upper tick gets negative liquidity_net", async () => {
      ...
      const tick = getTickData(ta, TICK_UPPER, upperArrayStart);
      expect(Number(tick.initialized)).to.equal(1);
  ...
  describe("cross_tick", () => {
    it("returns correct liquidity_net when crossing", async () => {
      ...
      const ta = await acct(program)["tickArray"].fetch(tickArrayPDA);
      const tick = getTickData(ta, 0, 0);
      const net = tick.liquidityNet as BN;

      expect(net.toString()).to.equal("1000000");
